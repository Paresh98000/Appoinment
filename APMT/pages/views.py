...
from pages.models import client,appointment,sp,admin_local,service,orders
from django.db.models import Q
from django.views.decorators.csrf import csrf_exempt

MERCHANT_KEY = "j15268EJU@ENH!to"
...
def getusername(req):
	...
	if clientid != None and clientid != '' and clientid != 'None':
		...
		if adminid != None and adminid != '' and adminid != 'None':
			issp = False
			isadmin = True
			print('\n\n',"select username from pages_admin_local where id = {}".format(adminid),'\n\n')
			row,c = qry("select username from pages_admin_local where id = {}".format(adminid))
			if row>0:
				return c.fetchone()[0]
			else:
				return None
...
def adminLogin(request):
	...
	if request.method == 'GET':
		...
		usr = admin_local.objects.filter(username=request.POST["name"],password=request.POST['password'])
		username = None
	...
	try:
		if request.COOKIES['userid'] == None or request.COOKIES['userid'] == '':
			...
			if request.method=="POST":
				pd = request.POST
				obj = appointment(id=(appointment.objects.count()+1),clientid=client.objects.filter(id=request.COOKIES['userid'])[0],serviceid=service.objects.filter(id=pd['serid'])[0],description=pd['detail'],date=pd['date'],time=pd['time'],payment_status="Not Paid")
				obj.save()
		
				return redirect('/homepage')
	except: 
		print('usen not signed in')
...
def manageappointment(request):
	...
	objs = appointment.objects.filter(clientid=uid,date__lt=dt1.now())
	objs2 = appointment.objects.filter(clientid=uid,date__gte=dt1.now())
	...
	if request.method == 'GET':
		return usercheck_profiling(request,'manageappointment.html',True,{'appointment_gone':objs,'appointment_left':objs2,"isleft":isexist2,"isgone":isexists})
	else:
		return usercheck_profiling(request,'manageappointment.html',True,{'appointment_gone':appointment.objects.filter(clientid=uid)})
...
def search(request):
	items = service.objects.filter(Q(title__icontains=request.GET['searchtext'])|Q(description__icontains=request.GET['searchtext'])|Q(stype__icontains=request.GET['searchtext'])|Q(city__icontains=request.GET['searchtext']))
	isitems = False
	if len(items)>0:
		isitems = True
...
@csrf_exempt
def order(request):
	usr = request.COOKIES['userid']

	if usr == None or usr == '' or usr == 'None' or usr == " ":
		return usercheck_profiling(request,'error.html',False,{"error":"Something whent wrong..."})

	appid = request.GET["id"]
	orderid = orders.objects.count()+1
	appoint = appointment.objects.filter(id=appid)
	amount = appoint[0].serviceid.cost
	custid = appoint[0].clientid.email
	obj = orders(id=orderid,amount=amount,appointmentid=appointment.objects.filter(id=appid)[0],status="Pending")
	obj.save()

	param_dict = {
        'MID': "JwQlcS02219862068658",
...
def handlerequest(request):
    ...
    if verify:
        if response_dict['RESPCODE'] == '01':
            orders.objects.filter(id=response_dict['ORDERID']).update(status="Paid")
            print('order successful')
        else:
            print('order was not successful because' + response_dict['RESPMSG'])
    return usercheck_profiling(request, 'paymentstatus.html',True, {'response': response_dict})	

def paytm(request):
	return usercheck_profiling(request,"paytm.html",True,{})

def aboutus(request):
	return usercheck_profiling(request,'aboutus.html')

def contactus(request):
	return usercheck_profiling(request,'contactus.html')